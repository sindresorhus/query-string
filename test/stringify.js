...
test('stringify', t => {
	...
	t.is(fn.stringify({foo: 'bar', bar: 'baz'}), 'bar=baz&foo=bar');
});
...
test('handle array value', t => {
	t.is(fn.stringify({abc: 'abc', foo: ['bar', 'baz']}), 'abc=abc&foo=bar&foo=baz');
});

test('array order', t => {
	t.is(fn.stringify({abc: 'abc', foo: ['baz', 'bar']}), 'abc=abc&foo=baz&foo=bar');
});

test('handle empty array value', t => {
	t.is(fn.stringify({abc: 'abc', foo: []}), 'abc=abc');
});

test('should not encode undefined values', t => {
	t.is(fn.stringify({abc: undefined, foo: 'baz'}), 'foo=baz');
});

test('should encode null values as just a key', t => {
	t.is(fn.stringify({'x y z': null, 'abc': null, 'foo': 'baz'}), 'abc&foo=baz&x%20y%20z');
});

test('handle null values in array', t => {
	t.is(fn.stringify({foo: null, bar: [null, 'baz']}), 'bar&bar=baz&foo');
});

test('handle undefined values in array', t => {
	t.is(fn.stringify({foo: null, bar: [undefined, 'baz']}), 'bar=baz&foo');
});

test('handle undefined and null values in array', t => {
	t.is(fn.stringify({foo: null, bar: [undefined, null, 'baz']}), 'bar&bar=baz&foo');
});
